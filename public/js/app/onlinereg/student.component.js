...
var studentApp = angular.module("student",["ngRoute",'ngMessages']).config(['$routeProvider','$mdDateLocaleProvider', function($routeProvider,$mdDateLocaleProvider) {
...
}]).controller("onlineregCtrl",function($timeout,$http,$scope,fileReader,$window,$httpParamSerializer){    
    var $ctrl = this;
     $ctrl.student= {};
     //$ctrl.student.classe ;
   ...
   $scope.dateChanged = function() {
    ...
    var data = {id: $ctrl.student.matricule};
    var config = {
    params: data,
    headers : {'Accept' : 'application/json'}
    };        
    $timeout($http.get('students',config).then(function(response){
     $ctrl.student = response.data[0];
     //$ctrl.student.photo ={};
     $ctrl.student.imageSrc = 'data:image/png;base64,'+$ctrl.student.photo.content;
    ...
    $timeout($http.get('getPreRegisteredStd',config).then(function(response){
     $ctrl.student = response.data[0];
     //$ctrl.student.photo ={};
     $ctrl.student.imageSrc = 'data:image/png;base64,'+$ctrl.student.photo.content;
    ...
    //End Countries 
        ...
        $timeout($http.get('semesterByClasse',config).then(
            ...
            $http.get('subjectsByClasse',config).then(
                ...
                if($scope.currentYrSubjects.length<=0) $scope.flag=-1;
